...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = WorkflowsResponse.from_dict(response.json()) if response.content else None

        return response_200

    if response.status_code == 401:
        response_401 = ListWorkflowsResponse401.from_dict(response.json()) if response.content else None

        return response_401

    if response.status_code == 500:
        response_500 = ListWorkflowsResponse500.from_dict(response.json()) if response.content else None
