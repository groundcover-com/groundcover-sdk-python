...
class TestSecretsLifecycle:
    ...
    def test_secret_crud(self, gc_client: groundcover.Client, tracker: ResourceTracker) -> None:
        # Create
        secret = tracker.new("secret")
        ...
        secret.resource_id = create_data["id"]
        secret_id = secret.resource_id
        assert secret_id
        assert create_data["name"] == secret.name
        ...
        tracker.forget(secret)

    def test_secret_types(self, gc_client: groundcover.Client, tracker: ResourceTracker) -> None:
        ...
        for secret_type in ("api_key", "password", "basic_auth"):
            ...
            secret.resource_id = data["id"]
            assert data["type"] == secret_type

    def test_create_secret_with_managed_provider(self, gc_client: groundcover.Client, tracker: ResourceTracker) -> None:
